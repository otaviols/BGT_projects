...
public class BOTA_Mirror_Puzzle_2 : BOTA_MissionEntity
{
  ...
  public override void PreloadAssets()
  {
    ...
    foreach (string soundPath in new List<string>()
    {
      ...
      (string) BOTA_Mirror_Puzzle_2.VO_BOTA_BOSS_03h_Female_Human_Whoops_01
    })
  ...
  {
    BOTA_Mirror_Puzzle_2 botaMirrorPuzzle2 = this;
    Actor enemyActor = GameState.Get().GetOpposingSidePlayer().GetHeroCard().GetActor();
    if (missionEvent == 99)
    {
      while (botaMirrorPuzzle2.m_enemySpeaking)
        yield return (object) null;
      GameState.Get().SetBusy(true);
      yield return (object) botaMirrorPuzzle2.PlayBossLine(enemyActor, botaMirrorPuzzle2.COMPLETE_LINE);
