...
public class QuestDialogOnProgress1DbfRecord : DbfRecord
{
  ...
  {
    QuestDialogOnProgress1DbfAsset progress1DbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof (QuestDialogOnProgress1DbfAsset)) as QuestDialogOnProgress1DbfAsset;
    if ((UnityEngine.Object) progress1DbfAsset == (UnityEngine.Object) null)
    {
      records = new List<T>();
      Debug.LogError((object) string.Format("QuestDialogOnProgress1DbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {0}", (object) resourcePath));
      return false;
    }
    for (int index = 0; index < progress1DbfAsset.Records.Count; ++index)
      progress1DbfAsset.Records[index].StripUnusedLocales();
    records = progress1DbfAsset.Records as List<T>;
    return true;
  }
