...
public class QuestDialogOnReceivedDbfRecord : DbfRecord
{
  ...
  {
    QuestDialogOnReceivedDbfAsset receivedDbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof (QuestDialogOnReceivedDbfAsset)) as QuestDialogOnReceivedDbfAsset;
    if ((UnityEngine.Object) receivedDbfAsset == (UnityEngine.Object) null)
    {
      records = new List<T>();
      Debug.LogError((object) string.Format("QuestDialogOnReceivedDbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {0}", (object) resourcePath));
      return false;
    }
    for (int index = 0; index < receivedDbfAsset.Records.Count; ++index)
      receivedDbfAsset.Records[index].StripUnusedLocales();
    records = receivedDbfAsset.Records as List<T>;
    return true;
  }
