using Blizzard.T5.Core.Utils;
using Blizzard.T5.MaterialService.Extensions;
using UnityEngine;

public class Stealth : MonoBehaviour
{
  public int RenderResolutionX = 256;
  public int RenderResolutionY = 256;
  public float AboveClip = 0.4f;
  public float BelowClip = 0.4f;
  private Camera stealthCamera;
  private GameObject stealthCameraGO;
  private RenderTexture tempRenderBuffer;
  private const int IgnoreLayer = 21;

  private void Start()
  {
    this.CreateCamera();
    this.CreateRenderTexture();
    this.SetupCamera();
    this.SetupMaterial();
  }

  private void OnDestroy() => RenderTexture.ReleaseTemporary(this.tempRenderBuffer);

  private void CameraRender() => this.stealthCamera.Render();

  private void CreateCamera()
  {
    if (!((Object) this.stealthCameraGO == (Object) null))
      return;
    if ((Object) this.stealthCamera != (Object) null)
      Object.Destroy((Object) this.stealthCamera);
    this.stealthCameraGO = new GameObject();
    this.stealthCamera = this.stealthCameraGO.AddComponent<Camera>();
    this.stealthCameraGO.name = this.name + "_StealthFXCamera";
    GameObjectUtils.SetHideFlags((Object) this.stealthCameraGO, HideFlags.HideAndDontSave);
  }

  private void SetupCamera()
  {
    this.stealthCamera.orthographic = true;
    this.UpdateOffScreenCamera();
    this.stealthCamera.transform.parent = this.transform;
    this.stealthCamera.nearClipPlane = -this.AboveClip;
    this.stealthCamera.farClipPlane = this.BelowClip;
    this.stealthCamera.targetTexture = this.tempRenderBuffer;
    this.stealthCamera.depth = Camera.main.depth - 1f;
    this.stealthCamera.backgroundColor = Color.black;
    this.stealthCamera.clearFlags = CameraClearFlags.Color;
    this.stealthCamera.cullingMask &= -2097153;
    this.stealthCamera.enabled = false;
    this.stealthCamera.allowHDR = false;
  }

  private void UpdateOffScreenCamera()
  {
    this.stealthCamera.orthographicSize = 1f;
    this.stealthCameraGO.transform.position = this.transform.position;
    this.stealthCameraGO.transform.rotation = this.transform.rotation;
    this.stealthCameraGO.transform.Rotate(90f, 180f, 0.0f);
  }

  private void CreateRenderTexture() => this.tempRenderBuffer = RenderTexture.GetTemporary(this.RenderResolutionX, this.RenderResolutionY);

  private void SetupMaterial() => this.gameObject.GetComponent<Renderer>().GetMaterial().mainTexture = (Texture) this.tempRenderBuffer;
}
