...
public class ArenaMedal : PegUIElement
{
  ...
  {
    this.m_medal = medal;
    AssetLoader.Get().LoadAsset<Texture>(ArenaMedal.s_arenaMedalTextures[medal], new AssetHandleCallback<Texture>(this.OnTextureLoaded));
  }
  ...
  private void MedalOut(UIEvent e) => this.gameObject.GetComponent<TooltipZone>().HideTooltip();
    ...
    object callbackData)
  {
    using (texture)
    {
      if (!(bool) texture)
      {
        Debug.LogWarning((object) string.Format("ArenaMedal.OnTextureLoaded(): asset for {0} is null!", (object) assetRef));
      }
      else
      {
        AssetHandle.Set<Texture>(ref this.m_medalTexture, texture);
        RendererExtension.GetMaterial(this.m_rankMedal.GetComponent<Renderer>()).mainTexture = (Texture) this.m_medalTexture;
      }
    }
  }

  private string GetMedalName() => GameStrings.Get("GLOBAL_ARENA_MEDAL_" + (object) this.m_medal);
