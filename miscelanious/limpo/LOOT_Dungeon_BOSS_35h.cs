...
public class LOOT_Dungeon_BOSS_35h : LOOT_Dungeon
{
  ...
  protected override string GetBossDeathLine() => "VO_LOOTA_BOSS_35h_Male_Furbolg_Death_01.prefab:044f0aa3de369464db7a3577780cf268";
  ...
  protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
  {
    ...
    {
      Gameplay.Get().StartCoroutine(this.PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_35h_Male_Furbolg_Intro_01.prefab:f6f44292c06355449ab33c49b87fd051", Notification.SpeechBubbleDirection.TopRight, actor));
    }
    else
    {
      if (!MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
        return;
      Gameplay.Get().StartCoroutine(this.PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_35h_Male_Furbolg_EmoteResponse_01.prefab:c01bc9067117b8142b96c8b60fe96b9d", Notification.SpeechBubbleDirection.TopRight, actor));
    }
  }

  protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
  {
    LOOT_Dungeon_BOSS_35h lootDungeonBoss35h = this;
    while (lootDungeonBoss35h.m_enemySpeaking)
      yield return (object) null;
    Actor enemyActor = GameState.Get().GetOpposingSidePlayer().GetHeroCard().GetActor();
    string str = "PLAYED_MISSION_EVENT_" + (object) missionEvent;
    if (!lootDungeonBoss35h.m_playedLines.Contains(str))
    {
      yield return (object) lootDungeonBoss35h.PlayLoyalSideKickBetrayal(missionEvent);
      if (missionEvent == 101)
      {
        int num1 = 30;
        int num2 = Random.Range(0, 100);
        if (lootDungeonBoss35h.m_CounteredLines.Count != 0 && num1 >= num2)
        {
          string randomLine = lootDungeonBoss35h.m_CounteredLines[Random.Range(0, lootDungeonBoss35h.m_CounteredLines.Count)];
          yield return (object) lootDungeonBoss35h.PlayLineOnlyOnce(enemyActor, randomLine);
          lootDungeonBoss35h.m_CounteredLines.Remove(randomLine);
          yield return (object) null;
