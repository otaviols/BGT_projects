...
public abstract class BOM_07_Scabbs_MissionEntity : GenericDungeonMissionEntity
{
  ...
  private static Map<GameEntityOption, bool> s_booleanOptions = BOM_07_Scabbs_MissionEntity.InitBooleanOptions();
  ...
  public BOM_07_Scabbs_MissionEntity() => this.m_gameOptions.AddBooleanOptions(BOM_07_Scabbs_MissionEntity.s_booleanOptions);
  ...
  {
    BOM_07_Scabbs_MissionEntity scabbsMissionEntity = this;
    float bossHeroPowerVoLine = scabbsMissionEntity.ChanceToPlayBossHeroPowerVOLine();
    float num = UnityEngine.Random.Range(0.0f, 1f);
    if (!scabbsMissionEntity.m_enemySpeaking && !scabbsMissionEntity.m_MissionDisableAutomaticVO && (double) bossHeroPowerVoLine >= (double) num && !((UnityEngine.Object) GameState.Get().GetOpposingSidePlayer().GetHero().GetCard().GetActor() == (UnityEngine.Object) null))
      yield return (object) scabbsMissionEntity.HandleMissionEventWithTiming(510);
  }

  protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
  {
    BOM_07_Scabbs_MissionEntity scabbsMissionEntity = this;
    if (!scabbsMissionEntity.m_MissionDisableAutomaticVO && !scabbsMissionEntity.m_enemySpeaking && entity.GetCardType() != TAG_CARDTYPE.INVALID && entity.GetCardType() == TAG_CARDTYPE.HERO_POWER && entity.GetControllerSide() == Player.Side.FRIENDLY)
      yield return (object) scabbsMissionEntity.HandleMissionEventWithTiming(508);
  }

  protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
  {
    BOM_07_Scabbs_MissionEntity scabbsMissionEntity = this;
    if (!scabbsMissionEntity.m_MissionDisableAutomaticVO && !scabbsMissionEntity.m_enemySpeaking && entity.GetCardType() != TAG_CARDTYPE.INVALID && entity.GetCardType() == TAG_CARDTYPE.HERO_POWER && entity.GetControllerSide() == Player.Side.OPPOSING)
      yield return (object) scabbsMissionEntity.OnBossHeroPowerPlayed(entity);
  }
  ...
  public override IEnumerator OnPlayThinkEmoteWithTiming()
  {
    ...
    if (!scabbsMissionEntity.m_enemySpeaking)
    {
      Player currentPlayer = GameState.Get().GetCurrentPlayer();
      if (currentPlayer.IsFriendlySide() && !currentPlayer.GetHeroCard().HasActiveEmoteSound())
      {
        GameState.Get().GetOpposingSidePlayer().GetHero().GetCardId();
        if ((scabbsMissionEntity.m_Mission_FriendlyPlayIdleLines || scabbsMissionEntity.m_Mission_EnemyPlayIdleLines) && (double) scabbsMissionEntity.GetThinkIdleChancePercentage() >= (double) UnityEngine.Random.Range(0.0f, 1f))
        {
          if (((double) scabbsMissionEntity.GetThinkEmoteBossIdleChancePercentage() < (double) UnityEngine.Random.Range(0.0f, 1f) || !scabbsMissionEntity.m_Mission_FriendlyPlayIdleLines) && scabbsMissionEntity.m_Mission_EnemyPlayIdleLines)
          {
            if (scabbsMissionEntity.m_Mission_EnemyPlayIdleLinesUseingEmoteSystem)
              yield return (object) scabbsMissionEntity.MissionPlayThinkEmote(actor1);
            else
              yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.HandleMissionEventWithTiming(517));
          }
          else if (scabbsMissionEntity.m_Mission_FriendlyPlayIdleLines)
          {
            if (scabbsMissionEntity.m_Mission_FriendlyPlayIdleLinesUseingEmoteSystem)
              yield return (object) scabbsMissionEntity.MissionPlayThinkEmote(actor2);
            else
              yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.HandleMissionEventWithTiming(518));
          }
        }
      }
    }
  ...
  {
    BOM_07_Scabbs_MissionEntity scabbsMissionEntity = this;
    while (scabbsMissionEntity.m_enemySpeaking)
      yield return (object) null;
    while (entity.GetCardType() == TAG_CARDTYPE.INVALID)
      yield return (object) null;
    if (GameState.Get().GetGameEntity().GetTag(GAME_TAG.PREVIOUS_PUZZLE_COMPLETED) == 0)
    {
      scabbsMissionEntity.MissionPause(true);
      yield return (object) scabbsMissionEntity.HandleMissionEventWithTiming(520);
      scabbsMissionEntity.MissionPause(false);
    }
  }
    ...
    MissionEntity.ShouldPlay shouldPlay)
  {
    ...
    {
      Notification.SpeechBubbleDirection speakerDirection = scabbsMissionEntity.GetDirection(actor);
      if (scabbsMissionEntity.m_forceAlwaysPlayLine)
        yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlayLine(actor, line, shouldPlay, 2.5f));
      ...
      if (shouldPlay() == scabbsMissionEntity.InternalShouldPlayAlways())
        yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlaySoundAndBlockSpeech(line, speakerDirection, actor, 2.5f, parentBubbleToActor: parentBubbleToActor));
      else if (shouldPlay() == scabbsMissionEntity.InternalShouldPlayOnlyOnce())
      {
        yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlaySoundAndBlockSpeechOnce(line, speakerDirection, actor, 2.5f, parentBubbleToActor: parentBubbleToActor));
  ...
  public IEnumerator MissionPlayVO(Actor actor, string line)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO(actor, line, true, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
    ...
    bool bPlayOrder = true)
  {
    ...
    if (shouldPlay() == MissionEntity.ShouldPlayValue.Once && !scabbsMissionEntity.m_forceAlwaysPlayLine)
      removeLine = true;
    string line = !bPlayOrder ? scabbsMissionEntity.PopNextLine(lines, removeLine) : scabbsMissionEntity.PopRandomLine(lines, removeLine);
    if (line != null)
    {
      yield return (object) scabbsMissionEntity.MissionPlayVO(speaker, line, bUseBubble, shouldPlay);
      scabbsMissionEntity.m_lastVOplayFinishtime = Time.time;
    }
  }
  ...
  public IEnumerator MissionPlayVO(Actor actor, List<string> lines)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO(actor, lines, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
  ...
  public IEnumerator MissionPlayVOOnce(Actor actor, List<string> lines)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO(actor, lines, new MissionEntity.ShouldPlay(((MissionEntity) scabbsMissionEntity).InternalShouldPlayOnlyOnce));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
    ...
    MissionEntity.ShouldPlay shouldPlay)
  {
    ...
    if (scabbsMissionEntity.m_enemySpeaking)
      yield return (object) null;
    scabbsMissionEntity.m_enemySpeaking = true;
    if (scabbsMissionEntity.m_forceAlwaysPlayLine)
      yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlayBigCharacterQuoteAndWait(brassRing, line));
    if (shouldPlay() == MissionEntity.ShouldPlayValue.Always)
      yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlayBigCharacterQuoteAndWait(brassRing, line));
    else if (shouldPlay() == MissionEntity.ShouldPlayValue.Once)
    {
      yield return (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlayBigCharacterQuoteAndWaitOnce(brassRing, line));
      NotificationManager.Get().ForceAddSoundToPlayedList(line);
    }
    scabbsMissionEntity.m_enemySpeaking = false;
  }
    ...
    bool bPlayOrder = true)
  {
    ...
    if (shouldPlay() == MissionEntity.ShouldPlayValue.Once && !scabbsMissionEntity.m_forceAlwaysPlayLine)
      removeLine = true;
    string line = !bPlayOrder ? scabbsMissionEntity.PopRandomLine(lines, removeLine) : scabbsMissionEntity.PopNextLine(lines, removeLine);
    yield return (object) scabbsMissionEntity.MissionPlayVO((string) brassRing, line, bUseBubble, shouldPlay);
  }
  ...
  public IEnumerator MissionPlayVO(AssetReference brassRing, string line)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO((string) brassRing, line, true, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
  ...
  public IEnumerator MissionPlayVO(AssetReference brassRing, List<string> lines)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO(brassRing, lines, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
  ...
  public IEnumerator MissionPlayVO(string minionSpeaker, List<string> lines)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO(scabbsMissionEntity.FindActorInPlayByDesignCode(minionSpeaker), lines, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
  ...
  public IEnumerator MissionPlayVO(string minionSpeaker, string line)
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) scabbsMissionEntity.MissionPlayVO(scabbsMissionEntity.FindActorInPlayByDesignCode(minionSpeaker), line, true, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
  ...
  {
    BOM_07_Scabbs_MissionEntity scabbsMissionEntity = this;
    if ((UnityEngine.Object) scabbsMissionEntity.FindActorInPlayByDesignCode(minionSpeaker) == (UnityEngine.Object) null)
      yield return (object) scabbsMissionEntity.MissionPlayVO((string) brassRing, line, true, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
    else
      yield return (object) scabbsMissionEntity.MissionPlayVO(scabbsMissionEntity.FindActorInPlayByDesignCode(minionSpeaker), line, true, new MissionEntity.ShouldPlay(scabbsMissionEntity.InternalShouldPlayAlways));
  }
  ...
  public IEnumerator MissionPlaySound(string line)
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) GameEntity.Coroutines.StartCoroutine(scabbsMissionEntity.PlaySoundAndWait(line, (string) null, Notification.SpeechBubbleDirection.None, (Actor) null, waitTimeScale, parentBubbleToActor, delayCardSoundSpells));
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
