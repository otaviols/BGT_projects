...
public class AdventureHeroPowerDbfRecord : DbfRecord
{
  ...
  {
    AdventureHeroPowerDbfAsset heroPowerDbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof (AdventureHeroPowerDbfAsset)) as AdventureHeroPowerDbfAsset;
    if ((UnityEngine.Object) heroPowerDbfAsset == (UnityEngine.Object) null)
    {
      records = new List<T>();
      Debug.LogError((object) string.Format("AdventureHeroPowerDbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {0}", (object) resourcePath));
      return false;
    }
    for (int index = 0; index < heroPowerDbfAsset.Records.Count; ++index)
      heroPowerDbfAsset.Records[index].StripUnusedLocales();
    records = heroPowerDbfAsset.Records as List<T>;
    return true;
  }
