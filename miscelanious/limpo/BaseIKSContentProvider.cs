...
public abstract class BaseIKSContentProvider
{
  ...
  public abstract IEnumerator GetQuery(
  ...
  public static T GetIntegerFromNode<T>(string keyName, JsonNode node)
  {
    ...
    return node[keyName] is string ? (T) Convert.ChangeType((object) int.Parse((string) node[keyName]), typeof (T)) : (T) Convert.ChangeType(node[keyName], typeof (T));
  }

  public static T GetStringFromNode<T>(string keyName, JsonNode node) => (T) (node.ContainsKey(keyName) ? node[keyName] : (object) string.Empty);
