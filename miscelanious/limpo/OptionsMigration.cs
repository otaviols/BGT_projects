...
public static class OptionsMigration
{
  ...
  public static bool UpgradeClientOptions()
  {
    ...
    for (; key < 4; ++key)
    {
      ...
      {
        Error.AddDevFatal("OptionsMigration.UpgradeClientOptions() - Current version is {0} and there is no function to upgrade to {1}. Latest is {2}.", (object) key, (object) (key + 1), (object) 4);
        return false;
      }
  ...
  private static bool UpgradeClientOptions_V3(int startingVersion)
  {
    ...
    float val = Mathf.Clamp(Options.Get().GetFloat(Option.MUSIC_VOLUME), 0.0f, 0.5f) / 0.5f;
    Options.Get().SetFloat(Option.MUSIC_VOLUME, val);
  ...
  public static bool UpgradeServerOptions()
  {
    ...
    for (; key < 6; ++key)
    {
      ...
      {
        Error.AddDevFatal("OptionsMigration.UpgradeServerOptions() - Current version is {0} and there is no function to upgrade to {1}. Latest is {2}.", (object) key, (object) (key + 1), (object) 6);
        return false;
      }
      if (!upgradeCallback(startingVersion))
        return false;
